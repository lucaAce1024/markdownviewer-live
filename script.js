...
<body>
  ...
  var lastLoadedTemplate = ''; // Track the last template we loaded into the editor

  function getLocalizedSampleMarkdown(lang) {
    if (sampleMarkdownCache[lang]) return sampleMarkdownCache[lang];

    // First, try to load the translated template from i18n JSON files
    if (window.I18n) {
      var translatedTemplate = I18n.t('defaultTemplate');
      if (translatedTemplate && translatedTemplate !== 'defaultTemplate') {
        sampleMarkdownCache[lang] = translatedTemplate;
        return translatedTemplate;
      }
    }

    // Fallback to the original English sample
    var lang = lang || 'en';
    sampleMarkdownCache[lang] = sampleMarkdown;
  ...
  function updateSampleMarkdownForLang() {
    ...
    // Clear cache to force re-fetch of translated template for new language
    sampleMarkdownCache = {};
    // Only update if the editor still has the default template or a localized version
    var currentContent = editor.value.trim();
    var isTemplate = !currentContent || currentContent === sampleMarkdown.trim() ||
      currentContent === lastLoadedTemplate.trim();
    if (isTemplate) {
      var localized = getLocalizedSampleMarkdown(lang);
      lastLoadedTemplate = localized;
